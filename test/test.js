...
describe('datacash', function() {
  describe('build', function() {
    ...
    describe('data and cash', function() {
      it('both data and cash', function(done) {
        ...
        datacash.build(options, function(err, tx) {
          ...
          let s1 = new btx.Script(generated.outputs[0].script)

          // the first output is OP_RETURN
          assert(s1.chunks[0].opcodenum, btx.Opcode.OP_RETURN)

          // the second script is a pubkeyhashout (change address)
          let s2 = new btx.Script(generated.outputs[1].script)
    ...
    describe('attach coins to data', function() {
      it('paying tip to 1 user', function(done) {
        // send to myself
        const receiver = new btx.PrivateKey(privKey).toAddress()
        ...
        datacash.build(options, function(err, tx) {
          ...
          // 1. OP_RETURN
          let s1 = new btx.Script(tx.outputs[0].script)
          assert(s1.chunks[0].opcodenum, btx.Opcode.OP_RETURN)
          // 2. Manual transaction output
          // the second script is a pubkeyhashout (change address)
          let s2 = new btx.Script(tx.outputs[1].script)
          assert(s2.isPublicKeyHashOut())
          // the value sent is 1000
          assert.equal(tx.outputs[1].satoshis, 1000)
          // the receiver address is the address specified in cash.to
          assert.equal(s2.toAddress().toString(), btx.Address(receiver).toString())

          // 3. Change address transaction output
          let s3 = new btx.Script(tx.outputs[2].script)
      ...
      it('paying tip to 2 users', function(done) {
        ...
        datacash.build(options, function(err, tx) {
          ...
          // the receiver address is the address specified in cash.to
          assert.equal(s3.toAddress().toString(), btx.Address(receiver).toString())

          // 3. Change address transaction output
          let s4 = new btx.Script(tx.outputs[3].script)
        ...
        it('tx + cash', function(done) {
          ...
          datacash.build(options1, function(err, tx1) {
            ...
            datacash.build(options2, function(err, tx2) {
              ...
              let script1 = new btx.Script(tx1.outputs[0].script)
              assert(script1.chunks[0].opcodenum, btx.Opcode.OP_RETURN)

              // tx2's output should have two items
              assert.equal(tx2.outputs.length, 2)
              let script2 = [
                new btx.Script(tx2.outputs[0].script),
                new btx.Script(tx2.outputs[1].script)
              ]
              // the first should be OP_RETURN
              assert(script2[0].chunks[0].opcodenum, btx.Opcode.OP_RETURN)
              // the second script is a pubkeyhashout (change address)
        ...
        it('tx only', function(done) {
          ...
          datacash.build(options1, function(err, tx1) {
            ...
            datacash.build({ tx: exported_tx1 }, function(err, tx2) {
              ...
              let script = new btx.Script(tx2.inputs[0].script)
  ...
  describe('advanced', function() {
    describe('btx', function() {
      it('exposes btx', function() {
        assert(datacash.btx.Networks)
        assert(datacash.btx.Opcode)
      })
    })
