...
var build = function(options, callback) {
  ...
  if (options.tx) {
    ...
    let tx = new btx.Transaction(options.tx)
  ...
  if (options.cash && options.cash.key) {
    ...
    const privateKey = new btx.PrivateKey(key);
    const address = privateKey.toAddress();
    const insight = new explorer.Insight(rpcaddr)
    insight.getUnspentUtxos(address, function (err, res) {
      let tx = new btx.Transaction(options.tx).from(res);
      if (script) {
        tx.addOutput(new btx.Transaction.Output({ script: script, satoshis: 0 }));
      }
    ...
    let tx = new btx.Transaction(options.tx).fee(fee);
    if (script) {
      tx.addOutput(new btx.Transaction.Output({ script: script, satoshis: 0 }));
    }
    callback(null, tx)
...
var _script = function(options) {
  ...
  if (options.data) {
    if (Array.isArray(options.data)) {
      ...
      s = btx.Script.fromHex(options.data);
    }
  }
  return s;
}
